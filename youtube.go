...
// From https://github.com/dustinblackman/speakerbot/
...
func searchYoutube(text string) (string, error) {
	formattedURL := fmt.Sprintf("https://www.googleapis.com/youtube/v3/search?part=snippet&q=%s&key=%s", url.QueryEscape(text), rundata.APITokens["youtube"])
